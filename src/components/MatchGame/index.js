...
class MatchGame extends Component {
  state = {
    currentImgObj: imagesList[0],
    activeTabId: tabsList[0].tabId,
    score: 0,
    isGameOver: false,
    remainingSec: 60,
    imageMatchUrl: imagesList[0].imageUrl,
  }
  ...
  componentDidMount() {
    ...
    // if (!this.timerID && this.state.isGameOver === false) {
    //   this.timerID = setInterval(this.tick, 1000)
    // }
    this.startTimer()
  }
  ...
  tick = () => {
    const {isGameOver, remainingSec} = this.state

    if (remainingSec > 0 && !isGameOver) {
      this.setState(prevState => ({
        remainingSec: prevState.remainingSec - 1,
      }))
    } else if (remainingSec === 0 && !isGameOver) {
  ...
  getRandomImage = () => {
    ...
    const randomImageObj = imagesList[randomIndex]
    // const currentImg = randomImageObj.imageUrl
    this.setState({
      currentImgObj: randomImageObj,
      imageMatchUrl: randomImageObj.imageUrl,
    })
  }
  ...
  getFilteredThumbnails = () => {
    ...
    const filteredThumbnails = imagesList.filter(
      eachThumbnailDetails => eachThumbnailDetails.category === activeTabId,
    )
    return filteredThumbnails
  }

  clickThumbnail = thumbnailId => {
    console.log('thumbnail clicks')
    const {currentImgObj} = this.state
    if (currentImgObj.id === thumbnailId) {
      this.setState(prevState => ({
  ...
  render() {
    ...
    // const matchImgSrc = currentImgObj.imageUrl
    const filteredThumbnails = this.getFilteredThumbnails()

    return (
      <div className="app-container pb-5">
        <Header score={score} remainingSec={remainingSec} />
        {isGameOver === true || remainingSec === 0 ? (
          <div className="main-container game-over-bg-container mt-5">
            <div className="gameover-container">
              ...
              />
              <div className="your-score-container d-flex flex-column">
                <p className="your-score-text mb-0">YOUR SCORE</p>
                <p className="game-over-score align-self-center mt-0">
                  {score}
                </p>
              </div>
              <button
          ...
          <div className="main-container mt-5">
            ...
            <ul className="thumbnails-container">
              {filteredThumbnails.map(eachThumbnail => (
                <ThumbnailItem
